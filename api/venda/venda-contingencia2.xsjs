...
function fnHandleGet(byId) {
    ...
    var query = ' SELECT ' +
		...
		' WHERE ' +
		'	1 = ? and tbv.STCANCELADO=\'False\' and tbv.STCONTINGENCIA=\'True\' ';
    ...
    if(dataFechamento) {
        query = query + ' AND tbv.DTHORAFECHAMENTO >= \'' + dataFechamento + '\' ';
    }
...
function fnHandlePut(){
    ...
    for (var i = 0; i < bodyJson.length; i++) {
		...
		var registro = bodyJson[i];
			...
			pStmt.setString(13, registro.NFE_INFNFE_IDE_SERIE);
			pStmt.setString(14, registro.NFE_INFNFE_IDE_NNF);
