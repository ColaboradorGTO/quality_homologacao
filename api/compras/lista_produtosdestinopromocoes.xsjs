...
function fnHandleGet(byId) {
    ...
    var query = ' SELECT ' + 
    '   tbt.IDDETALHEPROMOCAOMARKETINGDESTINO,' +
    '   tbt.IDRESUMOPROMOCAOMARKETING,' +
    '   tbt.IDGRUPOEMDESTINO,' +
    '   tbt.IDSUBGRUPOEMDESTINO,' +
    '   tbt.IDMARCAEMDESTINO,' +
    '   tbt.IDFORNECEDOREMDESTINO,' +
    '   tbt.IDPRODUTODESTINO,' +
    '   tbp.NUCODBARRAS AS NUCODBARRASDESTINO,' +
    '   tbp.DSNOME AS DSPRODUTODESTINO' +
    ' FROM ' + 
    '   "VAR_DB_NAME".DETALHEPROMOCAOMARKETINGDESTINO tbt' +
    '   INNER JOIN "VAR_DB_NAME"."PRODUTO" tbp on tbt.IDPRODUTODESTINO = tbp.IDPRODUTO ' +
    ' WHERE ' +
        '	1 = ?';
    
    if ( byId ) {
        query = query + ' And  tbt.IDDETALHEPROMOCAOMARKETINGDESTINO = \'' + byId + '\' ';
    }
...
function fnHandlePost() {
    ...
    var query2 = 'INSERT INTO "VAR_DB_NAME"."DETALHEPROMOCAOMARKETINGDESTINO" ' +
		" ( " +
	...
	for (var i = 0; i < bodyJson.length; i++) {
		...
		var queryId = api.executeScalar('SELECT IFNULL(MAX(TO_INT("IDDETALHEPROMOCAOMARKETINGDESTINO")),0) + 1 FROM "VAR_DB_NAME"."DETALHEPROMOCAOMARKETINGDESTINO" WHERE 1 = ? ', 1);
