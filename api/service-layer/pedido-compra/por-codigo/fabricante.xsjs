...
function postSl(data, session) {
    var response = slApi.post('/Manufacturers',data,session);
...
function executeFabricante(){
    var codFabricante = $.request.parameters.get("codFabricante");
    var query = 'SELECT '+
        		'T1."IDFABRICANTE", '+
            	'T1."DSFABRICANTE", '+
            	'T1."DTCADASTRO", '+
                ...
                ' FROM "QUALITY_CONC_HML"."FABRICANTE" T1 '+
               	'WHERE  1=? AND' +
        		' T1."IDSAP" is NULL AND (T1.STMIGRADOSAP = \'False\' OR T1.STMIGRADOSAP IS NULL) AND  T1."IDFABRICANTE" ='+parseInt(codFabricante);
