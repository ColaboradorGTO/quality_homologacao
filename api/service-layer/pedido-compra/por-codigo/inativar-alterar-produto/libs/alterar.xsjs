...
function executeProduto(){
   var codPedido = $.request.parameters.get("codPedido");
    var query = 'SELECT "IDDETALHEPRODUTOPEDIDO", '+
                ...
                'INNER JOIN "QUALITY_CONC_HML"."FORNECEDOR" T4 ON T4.IDFORNECEDOR = T3.IDFORNECEDOR '+
                ...
                'WHERE  1=? AND' +
        		 '(T1.STMIGRADOSAP IS NULL OR T1.STMIGRADOSAP = \'False\') AND T3.IDANDAMENTO = 5  AND T1."IDRESUMOPEDIDO" = '+parseInt(codPedido);
	//return query;
	...
	//	return Str_Json;
       patchSl(det.IDPRODCADASTRO,JSON.parse(Str_Json),session);
