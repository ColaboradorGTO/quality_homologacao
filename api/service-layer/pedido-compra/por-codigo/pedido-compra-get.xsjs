...
function executePedidoCompra(){
	...
	for (var i = 0; i < linhas.length; i++) {
		...
		var formaPagamento = api.sqlQuery('select top 1 "PymCode" AS ID from "SBO_GTO_TESTE1".CRD2 where "CardCode" = ? order by "LineNum" desc',det.IDFORNECEDORSAP);
	    var retCodigoComprador = api.sqlQuery('select "SlpCode" AS IDCOMPRADOR from "SBO_GTO_TESTE".OSLP where "U_Matricula" = ?',det.IDCOMPRADOR);
	    var cdComprador = -1;
	...
	/*	if(i === 0){
      ...
      // return 'aqui';
       var resultMigracao = api.sqlQuery('select "U_ID_VENDA_PDV" from "SBO_GTO_PRD".OPOR where 1=? AND "U_ID_VENDA_PDV" = \''+ det.IDRESUMOPEDIDO.toString()+'\'', 1);
	   if(resultMigracao.length > 0)
	   {
	       atualizaMigracaoPedidoCompra(det.IDRESUMOPEDIDO);
	   }*/
	}
    return 'nao foi encontrado registro';
