...
function validaSeExisteMaisDeUmProduto(idResumoPedido, idDetalheProdutoPedido){
    let query = `
		SELECT
            TBR.IDRESUMOPEDIDO,
            TBD.IDDETALHEPEDIDO,
...
/*function removerProdutoPedidoSAP(docEntry, idResumoPedido, idDetalheProdutoPedido){
    ...
    /*let pedidoAtual = slApi.getData('/PurchaseOrders('+docEntry+')', session);
    let dadosAtuais = JSON.parse(pedidoAtual.body.asString());
    let linhasExistentes = dadosAtuais.DocumentLines;
    let docLines = montarJsonDocumentLines(listaProdutosPedido, linhasExistentes);
    let jsonPedidoCompleto = montarJson(dadosAtuais, docLines)
    //return {docEntry, idResumoPedido, idDetalheProdutoPedido, listaProdutosPedido}
    
    /*jsonListaProdutosPedidoSAP["TaxExtension"] = {
        "Incoterms": 0,
        "MainUsage": 21
    }
   // return jsonListaProdutosPedidoSAP
   //return jsonPedidoCompleto
    if(jsonProdsQuePermanecerao.DocumentLines.length > 0){
        return patchSl(docEntry, jsonPedidoCompleto);
    }
