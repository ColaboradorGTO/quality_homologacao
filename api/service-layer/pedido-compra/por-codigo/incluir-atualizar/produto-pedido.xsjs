...
function atualizaMigracaoProduto(idProduto, idDetalhe){
    //return idProduto;
...
function executeProduto(){
   var codProdPedido = $.request.parameters.get("codProdPedido");
    var query = 'SELECT "IDDETALHEPRODUTOPEDIDO", '+
                ...
                'WHERE  1=? AND ' +
        		`T1.STCADASTRADO = 'True' AND (T1.STMIGRADOSAP <> 'True') AND (T1.STREPOSICAO <> 'True') AND T1."IDDETALHEPRODUTOPEDIDO" = ${parseInt(codProdPedido)}`;
	//return query;
	...
	for (var i = 0; i < linhas.length; i++) {
		...
		var retNcm = api.sqlQuery(`select top 1 "AbsEntry" AS NCM from ${dbNameSAP}.ONCM where replace("NcmCode",'.','') = ? order by "AbsEntry" desc `,det.NUNCM);
       ...
       var resp = postSl(JSON.parse(Str_Json),session);
       
       if(resp === 'True'){
           var resultMigracao = api.sqlQuery(`select "ItemCode" from ${dbNameSAP}.OITM where 1=? AND "ItemCode" = '${det.IDPRODCADASTRO}' `, 1);
           
            if(resultMigracao.length > 0){
                atualizaMigracaoProduto(det.IDPRODCADASTRO, det.IDDETALHEPRODUTOPEDIDO);
            }
       } else {
           let logSap = resp.error.message.value || resp.message['Store.store'];
    ...
    return {
        "type": 'success',
        "msg": 'Migração Produto realizada com sucesso!'
    };
}
if($.response) {
