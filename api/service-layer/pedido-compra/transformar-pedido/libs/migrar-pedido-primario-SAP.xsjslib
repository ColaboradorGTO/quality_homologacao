...
function updateLogSuccessTabelaPedidosComprasParaRecriar(idResumoPedido){
    let query = `
        ...
        SET
            ST_MIGRADO_SAP_PEDIDO_PRIMARIO = 'True',
            LOG_ERROR = NULL
        WHERE 
            IDRESUMOPEDIDO = ?
    `;
...
function updateLogErrorTabelaPedidosComprasParaRecriar(idResumoPedido, msgError){
    let query = `
        UPDATE 
            "VAR_DB_NAME".PEDIDOSCOMPRASRECRICADOS 
        SET
            ST_MIGRADO_SAP_PEDIDO_PRIMARIO = 'False',
            LOG_ERROR = ?
        WHERE 
            IDRESUMOPEDIDO = ?
    `;
...
function getIdResumoPedidoPrimario(idResumoPedido){
    let query = `
        SELECT
            "IDRESUMOPEDIDO"
        FROM 
...
function getDadosPedido(idResumoPedido){
    let query = `
        SELECT 
            ...
            TBR_PEDIDO_SECUNDARIO.IDRESUMOPEDIDO AS IDRESUMOPEDIDO_SECUNDARIO
        FROM 
        ...
        LEFT JOIN "VAR_DB_NAME"."RESUMOPEDIDO" TBR_PEDIDO_SECUNDARIO ON 
            TBR.IDRESUMOPEDIDO = TBR_PEDIDO_SECUNDARIO.IDPEDIDOPRIMARIO
        WHERE  
...
function montarJsonPedido(dados, BplIdCliente){
    ...
    let idResumoPedidoSecundario = Number(dados.IDRESUMOPEDIDO_SECUNDARIO || 0);
    ...
    if(idResumoPedidoSecundario > 0){
        comments += ` -> PEDIDO SECUNDARIO(${idResumoPedidoSecundario})`;
    }
...
function migrarPedidoPrimario(idResumoPedidoParaTransformar){
    ...
    try{
        ...
        if(dadosPedido.length > 0){
            ...
            for (let dados of dadosPedido) {
                let jsonPedido = montarJsonPedido(dados, idFilialIntermediaria);
                
                postSl(Number(dados.IDRESUMOPEDIDO), idResumoPedidoParaTransformar, jsonPedido);
            }
            
        } else {
            updateLogErrorTabelaPedidosComprasParaRecriar(idResumoPedidoParaTransformar, 'Erro ao tentar integrar o pedido primario, dados nao encontrados para integrar');
        }
    } catch(error){
        updateLogErrorTabelaPedidosComprasParaRecriar(idResumoPedidoParaTransformar, (error.message || 'Erro ao tentar migrar o pedido primario'));
