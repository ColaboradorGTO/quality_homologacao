...
function updateLogSuccessTabelaPedidosParaTransformar(idResumoPedido){
    let query = `
        ...
        SET
            ST_MIGRADO_SAP_PRODUTOS_PEDIDO_PRIMARIO = 'True',
            LOG_ERROR = NULL
        WHERE 
            IDRESUMOPEDIDO = ?
    `;
...
function updateLogErrorTabelaPedidosParaTransformar(idResumoPedido, msgError){
    let query = `
        UPDATE 
            "VAR_DB_NAME".PEDIDOSCOMPRASRECRICADOS 
        SET
            ST_MIGRADO_SAP_PRODUTOS_PEDIDO_PRIMARIO  = 'False',
            LOG_ERROR = ?
        WHERE 
            IDRESUMOPEDIDO = ?
    `;
...
function postSl(idDetalheProdutoPedido, idResumoPedidoParaTransformar, data) {
    ...
    if (response.status !== 204) {
        ...
        updateLogErrorTabelaPedidosParaTransformar(idResumoPedidoParaTransformar, (msgReturnError || `Erro ao migrar o produto de Reposicao do pedido primario para o SAP, IDPRODUTO: ${data.ItemCode}`));
    ...
    if(stMigrado){
        ...
        updateLogErrorTabelaPedidosParaTransformar(idResumoPedidoParaTransformar, `Erro ao migrar o produto de Reposicao do pedido primario para o SAP, IDPRODUTO: ${data.ItemCode}`);
...
function getDadosDetalheProdutoPedido(idResumoPedido){
    let query = `
        SELECT 
            ...
            (REPLACE(TBDPP."IDPRODCADASTRO", '_RN', '') || '_RN') AS "IDPRODCADASTRO",
            TBDPP."DSPRODUTO",
            TBDPP.VRCUSTO,
...
function montarJsonProduto(dadosProduto, stReposicao = false){
	let retNcm = getNcmSAP(dadosProduto.NUNCM);
	let arrayListasPrecos = getListasPrecos(dadosProduto, stReposicao);
	let idProduto = dadosProduto.IDPRODCADASTRO;
	let codBarras = 'SEM GTIN';
...
function migrarProdutosPedidoPrimario(idResumoPedidoParaTransformar){
    ...
    try{
        ...
        for (let dadosProduto of registros) {
            ...
            if(stMigradoSAP){
                ...
                postSl(idDetalheProdutoPedido, idResumoPedidoParaTransformar, jsonProduto);
            }
        }
    } catch(error){
        updateLogErrorTabelaPedidosParaTransformar(idResumoPedidoParaTransformar, (error.message || 'Erro ao migrar os produtos do pedido primario para o SAP'));
