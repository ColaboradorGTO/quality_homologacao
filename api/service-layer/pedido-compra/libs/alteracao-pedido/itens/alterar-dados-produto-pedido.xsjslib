...
function atualizaMigracaoProduto(idProduto){
...
function executeProdutosItem(codItemPedido){
    ...
    var query = 'SELECT "IDDETALHEPRODUTOPEDIDO", '+
                ...
                'WHERE  1=? AND' +
        		 ' T1."IDDETALHEPEDIDO" = '+parseInt(codItemPedido);
	//return query;
	...
	for (var i = 0; i < linhas.length; i++) {
		...
		//return Str_Json;
		//return det.IDPRODCADASTRO;
       var retPat = patchSl(det.IDPRODCADASTRO,JSON.parse(Str_Json),session);
       //return retPat;
       if(retPat === 'True'){
           var resultMigracao = api.sqlQuery('select "ItemCode" from "SBO_GTO_TESTE4".OITM where 1=? AND "ItemCode" = \''+ det.IDPRODCADASTRO.toString()+'\'', 1);
    	   if(resultMigracao.length > 0)
    	   {
    	       atualizaMigracaoProduto(det.IDPRODCADASTRO);
    	   }
       }else{
           return retPat;
       }
