...
function executeAtualizarDadosClienteIntegrado(dadosCliente, stMsgRetorno) {
    let queryCliente = `
        SELECT
            TOP 10
            TBR.IDVOUCHER,
    ...
    if(byId){
        queryCliente += ` AND TBC.IDCLIENTE = '${byId}' `;
    } else {
        queryCliente += `
    ...
    if(regCliente.length){
        ...
        for (let i = 0; i < regCliente.length; i++) {
            ...
            if(fnValidarDadosFiscaisCliente(registro) == true){
                ...
                if(fnValidaIntegracao(registro.IDCLIENTE, registro.NUCPFCNPJ) == false){
                    if(i == 0 ){
                    ...
                    postSl(dadosCliente, session, registro);
                } else {
                    if(stMsgRetorno){
                        return {
                ...
                if(stMsgRetorno){
                    return {
                        msg: "Cliente não integrado, dados fiscais inválidos!"
                    }
                }
            }
        }
...
function integrarCliente(dadosCliente) {
    let queryCliente = `
        SELECT
            IDCLIENTE,
            DSNOMERAZAOSOCIAL,
    ...
    if(dadosCliente.length){
        ...
        for (let registro of dadosCliente) {
            ...
            return PatchSl(dadosCliente, session, objClienteQuality)
        }
    }
...
if($.response) {
    ...
    try {
        switch ($.request.method) {
            ...
            var docReturn = executeIntegrarClienteDevolucao(id, stMsgRetorno);
