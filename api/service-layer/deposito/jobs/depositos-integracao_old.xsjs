...
function fnMontarJsonContasPagar(dadosDeposito){
    ...
    return {
       ...
       // "Comments": "Integração DEPOSITO Quality", //TEM QUE SER APROVADO PELO JULIANO AINDA
        "Remarks": DSHISTORIO,
...
function integrarDepositosNoSAP(byId) {
    ...
    if($.response) {
        idDaConta = $.request.parameters.get("idConta");
        dataPesquisaInicio = $.request.parameters.get("dataPesquisaInicio");
        dataPesquisaFim = $.request.parameters.get("dataPesquisaFim");
        dataCompInicio = $.request.parameters.get("dataCompInicio");
        dataCompFim = $.request.parameters.get("dataCompFim");
        dataMovInicio = $.request.parameters.get("datamovinicio");
        dataMovFim = $.request.parameters.get("datamovfim");
    }
    ...
    let query = ` 
        ...
        WHERE
            1 = ?
            AND (TO_DATE(TBD.DTMOVIMENTOCAIXA) BETWEEN '2025-06-01' AND '2025-06-30')
            ...
            --//AND TBE.IDEMPRESA <= 101 --//RETIRAR DA CONSULTA
    `;

	if (byId) {
		query += ` AND TBD.IDDEPOSITOLOJA = '${byId}' `;
	}
	
	if ( idDaConta ) {
        query += ` AND TBD.IDCONTABANCO = '${idDaConta}' `;
    }
	
	if(dataPesquisaInicio && dataPesquisaFim) {
        query += ` AND (TO_DATE(TBD.DTDEPOSITO) BETWEEN '${dataPesquisaInicio}' AND '${dataPesquisaFim}')`;
    }
    
    if(dataCompInicio && dataCompFim) {
        query += ` AND (TO_DATE(TBD.DTCOMPENSACAO) BETWEEN '${dataCompInicio}' AND '${dataCompFim}')`;
    }
    
    if(dataMovInicio && dataMovFim) {
        query += ` AND (TO_DATE(TBD.DTMOVIMENTOCAIXA) BETWEEN '${dataMovInicio}' AND '${dataMovFim}')`;
    }
	
    query += ' ORDER BY TBD.IDDEPOSITOLOJA, TBD.DTMOVIMENTOCAIXA';
	
    let resultadoDepositos = api.sqlQuery(query, 1);
	
    if(resultadoDepositos.length === 0){
        return { msg: "DEPOSITO JÁ INTEGRADO OU CANCELADO OU NÃO EXISTE!" };
    }
    /*return {resultadoDepositos,
        resultadoDepositosLeng: resultadoDepositos.length
    }*/
    if(resultadoDepositos.length > 0){
        ...
        for (let registro of resultadoDepositos) {
            let {
                IDDEPOSITOLOJA,
                DTDEPOSITO,
                DTCOMPENSACAO,
