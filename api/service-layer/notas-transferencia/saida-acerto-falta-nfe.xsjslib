...
var espelhoSaidaFalta = $.import("quality.concentrador_homologacao.api.service-layer.notas-transferencia.entrada-acerto-falta-nfe","espelhoSaidaFalta");
...
function obterLinhasDoDetalhe(idResumoOt, codigoDeposito) {
	...
	for (var i = 0; i < linhas.length; i++) {
		...
		var docLine = {
    			...
    			"Quantity": parseInt(det.QTDDIFERENCA),
...
function saidaAcertoFaltaNfe(idResumoOt,sessionSl) {
	...
	for (var i = 0; i < response.length; i++) {
		...
		var retCardCode = api.sqlQuery('select "U_IS_PN_SAIDA" from "SBO_GTO_TESTE1".OBPL WHERE "BPLId" = ?',det.IDEMPRESAORIGEM); 
        var retSequenceCode = api.sqlQuery('select "SeqCode" as SEQCOD from "SBO_GTO_TESTE1".NFN1 where "Model" = 39 and "Locked" = \'N\' and "BPLId" = ?',det.IDEMPRESADESTINO); 
		var transferenciaSaida = {
				...
				"Comments": "Integração Quality - acerto por falta",
				"JournalMemo": "NFe de transf. entre filiais - acerto por falta",
        ...
        espelhoSaidaFalta.entradaAcertoFaltaNfe(det.IDRESUMOOT, sessionSl);
