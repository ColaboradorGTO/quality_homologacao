...
function fnAtualizaStatusVenda(idVenda, docEntrySAP, conn){
    let query = `
        UPDATE 
            "VAR_DB_NAME"."VENDA" 
        SET
    ...
    `;
 	...
 	pStmt.setInt(1, parseInt(docEntrySAP));
...
function fnAtualizarStatusDevolucaoResumoVoucher(idVoucher, docEntryDev, conn){
    let queryUpdate = `
        UPDATE
...
function fnAtualizarStatusREFDevolucaoResumoVoucher(idVoucher, conn){
    let queryUpdate = `
        UPDATE
            "VAR_DB_NAME"."RESUMOVOUCHER"
        SET 
...
function errorLogDevolucaoVenda(idVoucher, msg_Error, conn){
    let queryUpdate = `
        UPDATE
            "VAR_DB_NAME"."RESUMOVOUCHER"
        SET 
...
function fnVerificarIntegracaoVenda(idVenda, conn){
    ...
    if(regVenda.length){
        ...
        if(!regVendaMigrada.length){
            fnAtualizaStatusVenda(idVenda, regVenda[0]["DocEntry"], conn);
        }
...
function fnVerificarIntegracaoCliente(dadosCliente){
    let {
        IDCLIENTE, 
        NUCPFCNPJ, 
        EENDERECO, 
...
function executeGerarDevolucao(byId, stMsgRetorno = false){
    return {
        msg: 'teste'
    }
