...
function postSl(dadosVenda, session) {
    if(dadosVenda) {
        let response;
        let resultMigracao;
        let data = dadosVenda;
        let idVenda = data.U_ID_VENDA_PDV;
        let conn = $.db.getConnection();
        
        response = slApi.post('/Invoices',data,session);
        
        if(response.status != 204){
            response = JSON.parse(response.body.asString());
            if(response.error){
                let msgReturnError = translate.traduzirTexto(response.error.message.value);
                errorLogVenda(idVenda, msgReturnError, conn);
        ...
        let queryValidaMigracao = `
            ...
            WHERE
                TBO."CANCELED" = 'N' 
                AND TBI."MainUsage" = 38 
                AND TBO."U_ID_VENDA_PDV" = ?
        `;
        
        resultMigracao = api.sqlQuery(queryValidaMigracao, data.U_ID_VENDA_PDV);
     
        if(resultMigracao.length > 0){
            let idSap = resultMigracao[0].IDSAP;
            
            atualizaMigracaoVenda(idVenda, idSap, conn);
        }
...
function errorLogVenda(idVenda, p_Error, conn){
    let query = `
        UPDATE 
            "VAR_DB_NAME"."VENDA" 
        SET 
...
function atualizaMigracaoVenda(idVenda, idSap, conn){
    let query = `
        UPDATE 
            "VAR_DB_NAME"."VENDA" 
        SET
