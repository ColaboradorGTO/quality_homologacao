...
function fnHandleGet(byId) {
    ...
    if (dataPesquisaInicio) {
        query = query + ' AND DTHORAINICIO >= \'' + dataPesquisaInicio + ' 00:00:00\' ';
    }
    if (dataPesquisaFim) {
        query = query + ' AND DTHORAFIM <= \'' + dataPesquisaFim + ' 23:59:59\' ';
    }
    ...
    // Ordenar pelo IDRESUMOPROMOCAOMARKETING
    query += ' ORDER BY IDRESUMOPROMOCAOMARKETING DESC';
...
function incluirDetalhePromocaoMarketingDestino(conn, idResumoPromocao, idGrupo, idSubGrupoDestino, idMarca, idFornecedor, idProdutos, stEmpresaDestino) {
    ...
    var subGrupos = Array.isArray(idSubGrupoDestino) ? idSubGrupoDestino : [idSubGrupoDestino];
    ...
    for (var i = 0; i < subGrupos.length; i++) {
        ...
        var idSubGrupoDestinos = subGrupos[i];

        pStmt.setInt(1, Id);
        pStmt.setInt(2, idResumoPromocao);
        pStmt.setInt(3, idGrupo || -1);
        pStmt.setInt(4, idSubGrupoDestinos);
        pStmt.setInt(5, idMarca || -1);
        pStmt.setInt(6, idFornecedor || -1);
        setStringOrNull(pStmt, 7, idProdutos);
...
function incluirDetalhePromocaoMarketingOrigem(conn, idResumoPromocao, idGrupo, idSubGrupoOrigem, idMarca, idFornecedor, idProdutosOrigem, stEmpresaOrigem) {
    ...
    // idSubGrupoOrigem pode ser um array ou um único valor
    var subGruposOrigem = Array.isArray(idSubGrupoOrigem) ? idSubGrupoOrigem : [idSubGrupoOrigem];

    for (var i = 0; i < subGruposOrigem.length; i++) {
        ...
        var idSubGrupoOrigens = subGruposOrigem[i];

        pStmt.setInt(1, Id);
        pStmt.setInt(2, idResumoPromocao);
        pStmt.setInt(3, idGrupo || -1);
        pStmt.setInt(4, idSubGrupoOrigens);
        pStmt.setInt(5, idMarca || -1);
        pStmt.setInt(6, idFornecedor || -1);
        setStringOrNull(pStmt, 7, idProdutosOrigem);
...
function fnHandlePut() {
    ...
    try {
        ...
        for (var i = 0; i < bodyJson.length; i++) {
            ...
            setIntOrNull(pStmt, 15, registro.IDMECANICARESUMOPROMOCAOMARKETING);
