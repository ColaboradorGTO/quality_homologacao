...
function execute () {
    ...
    var dataAtualizacao = data.getDate()+'.'+data.getMonth()+'.'+data.getFullYear();
    //return dataAtualizacao;
    var queryProdutos = 'UPDATE "VAR_DB_NAME".PRODUTO SET '+
                        '    IDGRUPOEMPRESARIAL = itm."U_GRP_EMP" '+
                        '    ,NUNCM = Replace(Replace(ncm."NcmCode",\'.\',\'\'),\'-\',\'\') '+
                        '    ,NUCODBARRAS = itm."CodeBars" '+
                        '    ,DSNOME = itm."ItemName" '+ 
                        '    ,UND = itm."SalUnitMsr" '+
                        '    ,PRECOVENDA = itm1."Price" '+
                        '    ,DTULTALTERACAO = NOW()  '+
                        '    ,SKUVTEX = itm."U_AS_SKUVTEX" '+
                        '    ,IDFORNECEDORSAP = itm."CardCode" '+
                        ' FROM '+
                        '    "VAR_DB_NAME".PRODUTO, '+
                        ...
                        ' WHERE IDPRODUTO = itm."ItemCode" '+ 
                        '    AND itm1."PriceList" = 1  '+
                        '    AND ITW."WhsCode" =\'001\'  '+
                        '    AND itm."SellItem" =\'Y\'  '+
                        '    AND itm."UpdateDate" = \''+dataAtualizacao+'\'';
