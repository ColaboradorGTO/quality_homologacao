...
function fnHandleGet() {
    ...
    ' T.IDFUNCIONARIO, '+
    ' T.NOFUNCIONARIO, '+
    ' MAX(T.QTD_Vendas) AS QTD_Vendas, '+ 
    ' T.QTD_PRODUTO, '+ 
	' T.VRTOTALVENDA, '+ 
    ...
    ' F.IDFUNCIONARIO, '+
    ' F.NOFUNCIONARIO, '+
    ' DENSE_RANK() OVER (PARTITION BY VWV.IDOPERADOR ORDER BY VWV.IDVENDA ASC) AS QTD_Vendas, '+ 
	' SUM(VWV."QTD")OVER(PARTITION BY VWV.IDOPERADOR) as QTD_PRODUTO, '+ 
	' SUM(VWV."VRTOTALLIQUIDO")OVER(PARTITION BY VWV.IDOPERADOR) as VRTOTALVENDA, '+ 
	' SUM(VWV."PRECO_COMPRA")OVER(PARTITION BY VWV.IDOPERADOR) as VRCUSTOTOTAL '+ 
    ...
    query2 = query2 + ' ) AS T GROUP BY T.IDEMPRESA, T.NOFANTASIA, T.QTD_PRODUTO, T.VRTOTALVENDA,T.VRCUSTOTOTAL,T.IDFUNCIONARIO,T.NOFUNCIONARIO ORDER BY T.IDEMPRESA, T.IDFUNCIONARIO ';
