...
function fnHandlePost() {
    ...
    var pIdResumoOT = api.executeScalar('SELECT "VAR_DB_NAME".SEQ_RESUMOORDEMTRANSFERENCIA.NEXTVAL FROM DUMMY WHERE 1 = ?', 1);
...
function fnHandlePut(){
    ...
    var bodyJson = JSON.parse($.request.body.asString());
    if (!Array.isArray(bodyJson)) bodyJson = [bodyJson];
    var pIdStatusOT = parseInt(bodyJson.IDSTATUSOT);
    var pIdResumoOT = parseInt(bodyJson.IDRESUMOOT);
    ...
    if(pIdStatusOT === 1){
        var query1 = 'UPDATE "VAR_DB_NAME"."RESUMOORDEMTRANSFERENCIA" SET ' +
                	...
                	' WHERE "IDRESUMOOT" = ? ';
        var pStmt1 = conn.prepareStatement(api.replaceDbName(query1));
        pStmt1.setInt(1, bodyJson.NUTOTALITENS);
        pStmt1.setInt(2, bodyJson.QTDTOTALITENS);
        pStmt1.setFloat(3, bodyJson.VRTOTALCUSTO);
        pStmt1.setFloat(4, bodyJson.VRTOTALVENDA);
        pStmt1.setInt(5, pIdResumoOT);
    	pStmt1.execute();
        pStmt1.close();
    ...
    if(pIdStatusOT === 2){
        var query2 = 'UPDATE "VAR_DB_NAME"."RESUMOORDEMTRANSFERENCIA" SET ' +
                	...
                	' WHERE "IDRESUMOOT" = ? ';
        var pStmt2 = conn.prepareStatement(api.replaceDbName(query2));
        pStmt2.setInt(1, bodyJson.IDUSRCANCELAMENTO);
        pStmt2.setInt(2, pIdStatusOT);
        pStmt2.setInt(3, pIdResumoOT);
    	pStmt2.execute();
        pStmt2.close();
    ...
    if(pIdStatusOT === 4){
        ...
        var query4 = 'UPDATE "VAR_DB_NAME"."RESUMOORDEMTRANSFERENCIA" SET ' +
                	...
                	' WHERE "IDRESUMOOT" = ? ';
        var pStmt4 = conn.prepareStatement(api.replaceDbName(query4));
        pStmt4.setInt(1, bodyJson.QTDTOTALITENSRECEPCIONADO);
        pStmt4.setInt(2, bodyJson.IDOPERADORRECEPTOR);
        pStmt4.setInt(3, bodyJson.IDRESUMOOT);
    	pStmt4.execute();
        pStmt4.close();
    	...
    	for (var i = 0; i < bodyJson.dadosdetalheot.length; i++) {
    	    
    	    var pIdProduto = bodyJson.dadosdetalheot[i].IDPRODUTO;

            if (api.executeScalar('SELECT COUNT(1) FROM "VAR_DB_NAME".DETALHEORDEMTRANSFERENCIA WHERE STATIVO = \'True\' AND IDRESUMOOT = ' + pIdResumoOT + ' AND IDPRODUTO = ?', pIdProduto) > 0) {
                pStmtdetupd4.setInt(1, bodyJson.dadosdetalheot[i].QTDRECEPCAO);
                pStmtdetupd4.setString(2, bodyJson.dadosdetalheot[i].STCONFERIDO);
                pStmtdetupd4.setInt(3, pIdResumoOT);
                pStmtdetupd4.setString(4, pIdProduto);
                pStmtdetupd4.execute();
            } else {
        	    ...
        	    pStmtdetins4.setString(1, pIdProduto);
        	    pStmtdetins4.setInt(2, pIdResumoOT);
                pStmtdetins4.setInt(3, bodyJson.dadosdetalheot[i].QTDEXPEDICAO);
                pStmtdetins4.setInt(4, bodyJson.dadosdetalheot[i].QTDRECEPCAO);
                pStmtdetins4.setInt(5, bodyJson.dadosdetalheot[i].QTDDIFERENCA);
                pStmtdetins4.setInt(6, bodyJson.dadosdetalheot[i].QTDAJUSTE);
                pStmtdetins4.setFloat(7, bodyJson.dadosdetalheot[i].VLRUNITVENDA);
                pStmtdetins4.setFloat(8, bodyJson.dadosdetalheot[i].VLRUNITCUSTO);
                pStmtdetins4.setString(9, bodyJson.dadosdetalheot[i].STCONFERIDO);
                setIntOrNull(pStmtdetins4, 10, bodyJson.dadosdetalheot[i].IDUSRAJUSTE);
                pStmtdetins4.setString(11, bodyJson.dadosdetalheot[i].STATIVO);
                pStmtdetins4.setString(12, bodyJson.dadosdetalheot[i].STFALTA);
                pStmtdetins4.setString(13, bodyJson.dadosdetalheot[i].STSOBRA);
                pStmtdetins4.execute();
                pStmtdetins4.close();
            }
    	}
    ...
    // rotina para excluir produtos que foram incluídos na recepção
    if(pIdStatusOT === 5){
        if (api.executeScalar('SELECT COUNT(1) FROM "VAR_DB_NAME".DETALHEORDEMTRANSFERENCIA WHERE STATIVO = \'True\' AND IDRESUMOOT = ' + pIdResumoOT + ' AND IDPRODUTO = ?', bodyJson.IDPRODUTO) > 0) {
            var querydetdel5 = 'DELETE FROM "VAR_DB_NAME".DETALHEORDEMTRANSFERENCIA WHERE IDRESUMOOT = ? AND IDPRODUTO = ?';
        	var pStmtdetdel5 = conn.prepareStatement(api.replaceDbName(querydetdel5));
        	pStmtdetdel5.setInt(1, pIdResumoOT);
        	pStmtdetdel5.setString(2, bodyJson.IDPRODUTO);
        	pStmtdetdel5.execute();
        	pStmtdetdel5.close();
        }
    }
    ...
    if(pIdStatusOT === 6){
        ...
        var pStmt6 = conn.prepareCall(query6);
        pStmt6.setInt(1, pIdResumoOT);
        pStmt6.setInt(2, bodyJson.IDOPERADORRECEPTOR);
        pStmt6.setInt(3, bodyJson.QTDCONFERENCIA);
        pStmt6.execute();
        pStmt6.close();
    }
    ...
    if(pIdStatusOT === 7){
    	...
    	for (i = 0; i < bodyJson.dadosdetalheot.length; i++) {
    	    
    	    pIdProduto = bodyJson.dadosdetalheot[i].IDPRODUTO;

            if (api.executeScalar('SELECT COUNT(1) FROM "VAR_DB_NAME".DETALHEORDEMTRANSFERENCIA WHERE STATIVO = \'True\' AND IDRESUMOOT = ' + pIdResumoOT + ' AND IDPRODUTO = ?', pIdProduto) > 0) {
                pStmtdetupd7.setInt(1, bodyJson.dadosdetalheot[i].QTDAJUSTE);
                pStmtdetupd7.setInt(2, bodyJson.dadosdetalheot[i].IDUSRAJUSTE);
                pStmtdetupd7.setInt(3, pIdResumoOT);
                pStmtdetupd7.setString(4, pIdProduto);
                pStmtdetupd7.execute();
            } else {
        	    ...
        	    pStmtdetins7.setString(1, pIdProduto);
        	    pStmtdetins7.setInt(2, pIdResumoOT);
                pStmtdetins7.setInt(3, bodyJson.dadosdetalheot[i].QTDEXPEDICAO);
                pStmtdetins7.setInt(4, bodyJson.dadosdetalheot[i].QTDRECEPCAO);
                pStmtdetins7.setInt(5, bodyJson.dadosdetalheot[i].QTDDIFERENCA);
                pStmtdetins7.setInt(6, bodyJson.dadosdetalheot[i].QTDAJUSTE);
                pStmtdetins7.setFloat(7, bodyJson.dadosdetalheot[i].VLRUNITVENDA);
                pStmtdetins7.setFloat(8, bodyJson.dadosdetalheot[i].VLRUNITCUSTO);
                pStmtdetins7.setString(9, bodyJson.dadosdetalheot[i].STCONFERIDO);
                setIntOrNull(pStmtdetins7, 10, bodyJson.dadosdetalheot[i].IDUSRAJUSTE);
                pStmtdetins7.setString(11, bodyJson.dadosdetalheot[i].STATIVO);
                pStmtdetins7.setString(12, bodyJson.dadosdetalheot[i].STFALTA);
                pStmtdetins7.setString(13, bodyJson.dadosdetalheot[i].STSOBRA);
    ...
    if(pIdStatusOT === 8){
        ...
        var pStmt8 = conn.prepareCall(query8);
        pStmt8.setInt(1, pIdResumoOT);
        pStmt8.setInt(2, bodyJson.IDUSRAJUSTE);
        pStmt8.setInt(3, bodyJson.IDSTDIVERGENCIA);
        pStmt8.setString(4, bodyJson.OBSDIVERGENCIA);
        pStmt8.execute();
        pStmt8.close();
    }
