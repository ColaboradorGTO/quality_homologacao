...
function fnHandleGet(byId) {
	...
	var query = ' SELECT DISTINCT' +
		...
		'	RIGHT JOIN "VAR_DB_NAME".EMPRESA tbe  ON tbc.IDEMPRESA = tbe.IDEMPRESA  ' +
		' WHERE ' +
		'	1 = ?';
	...
	if(idEmpresa){
	    query = query + ' and tbe.IDEMPRESA = ' + idEmpresa;
	}
...
function fnHandlePut() {
	...
	for (var i = 0; i < bodyJson.length; i++) {
        ...
        pStmt.setString(1, registro.DSCAIXAWEB);
		pStmt.setString(2, registro.TBEMISSAOFISCAL);
		pStmt.setString(3, registro.NOIMPRESSORA);
		pStmt.setString(4, registro.DSPORTACOMUNICACAO);
		pStmt.setDate(5, registro.DTULTALTERACAO);
		pStmt.setInt(6, registro.NUSERIEPROD);
		pStmt.setInt(7, registro.NUNFCEPROD);
		pStmt.setString(8, registro.STTEF);
		pStmt.setString(9, registro.STATUALIZA);
		pStmt.setString(10, registro.STLIMPA);
		pStmt.setInt(11, registro.IDCAIXAWEB);
...
function fnHandlePost() {
	...
	for (var i = 0; i < bodyJson.length; i++) {
		...
		pStmt.setString(32, registro.STATUALIZA); 
		pStmt.setString(33, registro.STLIMPA);
