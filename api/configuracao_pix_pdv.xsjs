...
function fnHandleGet(byId) {
    ...
    var idPixPgtoVenda = $.request.parameters.get("idPixPgtoVenda");
    var idPixPgtoFatura = $.request.parameters.get("idPixPgtoFatura");
	...
	if(idPixPgtoVenda){
	    query = query + ' and tbc.IDPSPPIX = ' + idPixPgtoVenda;
	}
	
	if(idPixPgtoFatura){
	    query = query + ' and tbc.IDPSPPIXFATURA = ' + idPixPgtoFatura;
	}
...
function fnHandlePut() {
    ...
    let query = `UPDATE "VAR_DB_NAME".CONFIGURACAO
                  SET IDPSPPIX = ?,
                  IDPSPPIXFATURA = ?
    	         WHERE IDEMPRESA =  ? `;
    ...
    for (var i = 0; i < bodyJson.length; i++) {
        ...
        pStmt.setInt(1, registro.IDPSPPIX);
        pStmt.setInt(2, registro.IDPSPPIXFATURA);
...
function fnHandlePost() {
    ...
    var query = 'INSERT INTO "VAR_DB_NAME"."CONFIGURACAO" ' +
        ...
        ' "ST_SAP_ONLINE" '+ 
    	' ) ' +
		' VALUES(QUALITY_CONC.SEQ_CONFIGURACAO.NEXTVAL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ';
