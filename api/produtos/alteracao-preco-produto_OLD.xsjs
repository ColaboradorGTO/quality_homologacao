...
function criarResumoAlteracaoPreco(bodyJson, dtHoraResumoAlteracao, agendAlteracao, stExecutado, listEmpresas, conn){
    ...
    let query = `
		...
		VALUES(?,?,'${dtHoraResumoAlteracao}',?,?,'${agendAlteracao}', 'False', '${stExecutado}') 
	`;
...
function criarDetalheAlteracaoPreco(idResumoAlteracao, bodyJson, agendAlteracao, stExecutado, listEmpresas, conn){
    ...
    prodsAgrupadosPorPreco.map((dados, indice)=>{
        ...
        dados.map((dadoProd, i)=>{
            ...
            qtdEstoqueAnterior = Number(dadoProd.QTDESTOQUEAOCADASTRAR);
            qtdEstoque = Number(dadoProd.QTDESTOQUEAOEXECUTAR);
            ...
            criarHistoricoPrecoEmpresa(idResumoAlteracao, listEmpresas, stExecutado, dadoProd.PRECOVENDANOVO, dadoProd.IDPRODUTO, conn);
            
            if(stExecutado == 'True'){
                if(tipoAltercao == '0'){
                    criarHistoricoPrecoLista(idResumoAlteracao, idListaPrecoProds, dadoProd.PRECOVENDANOVO, dadoProd.IDPRODUTO, conn);
                }
...
function criarHistoricoPrecoLista(idResumoAlteracao, idListaPrecoProds, precoNovo, idProduto, conn, qtdEstoqueAtual){
    let precoAntigo = '';
    let qtdEstoqueAnterior = 0;
    let qtdEstoque = 0;
    let contador = 0;
...
function criarHistoricoPrecoEmpresa(idResumoAlteracao, idsEmpresas, stExecutado, precoNovo, idProduto, conn){
    ...
    if(idsEmpresas.length > 1){
        ...
        for(let i = 0; i < idsEmpresas.length; i++){
            ...
            if(i == idsEmpresas.length - 1 || contador >= 500){
            ...
            if(contador >= 500 || (i == idsEmpresas.length - 1)){
    ...
    /*prodsAgrupadosPorPreco.map((dados, indice)=>{
        ...
        dados.map((dadoProd, i)=>{
                idEmpresaProds = dadoProd.IDEMPRESA;
                idProdsPorPreco = dadoProd.IDPRODUTO;
                precoProd = Number(dadoProd.PRECOVENDANOVO);
                qtdEstoqueAnterior = Number(dadoProd.QTDESTOQUEAOEXECUTAR);
                qtdEstoque = Number(dadoProd.QTDESTOQUEAOCADASTRAR);
                
                
                // Busca o Preco Atual antes de ser modificado
                precoAntigo =  Number(buscaPrecoAtualEmpresa(dadoProd.IDPRODUTO, idEmpresaProds).PRECOVENDA);
                
                if(i == dados.length - 1 || contador >= 500){
                    queryInsertHist += ` SELECT ${idResumoAlteracao}, ${idEmpresaProds}, '${idProdsPorPreco}', ${precoAntigo}, ${precoProd}, ${qtdEstoqueAnterior}, ${qtdEstoque}, ${dtExecutado}, CURRENT_TIMESTAMP, '${stExecutado}' FROM DUMMY `;
                } else if(i < dados.length - 1){
                    queryInsertHist += ` SELECT ${idResumoAlteracao}, ${idEmpresaProds}, '${idProdsPorPreco}', ${precoAntigo}, ${precoProd}, ${qtdEstoqueAnterior}, ${qtdEstoque}, ${dtExecutado}, CURRENT_TIMESTAMP, '${stExecutado}'  FROM DUMMY `;
                    queryInsertHist += ` UNION ALL `;
                }else{
    ...
    listProds.map((prod) => {
        ...
        if(idListaPreco){
            queryQtdEstoqueProd = `
                ...
                SELECT
                    0 AS QTDESTOQUEAOCADASTRAR,
                    0 AS QTDESTOQUEAOEXECUTAR
                FROM
                    DUMMY
                WHERE
                    1 = ?
        ...
        prod.QTDESTOQUEAOCADASTRAR = Number(respRegProd[0].QTDESTOQUEAOCADASTRAR);
        return prod.QTDESTOQUEAOEXECUTAR = Number(respRegProd[0].QTDESTOQUEAOEXECUTAR); 
    })
