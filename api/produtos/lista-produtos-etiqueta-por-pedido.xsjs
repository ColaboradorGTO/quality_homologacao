...
function fnHandleGet(byId) {
    ...
    query = `
        SELECT --DISTINCT
            ...
            CASE
                ...
                WHEN IFNULL(TBT.DSTAMANHO, '') = '' AND IFNULL(SPOITM.U_TAM, '') <> '' THEN SPOITM.U_TAM
                ELSE (
                    SELECT
                        "Name"
                    FROM
            ...
            CASE
                ...
                WHEN TBGS.IDGRUPOESTRUTURA = 9 THEN 'Inimo' || '-' || D."U_Desc" || '-' || TBES. DSESTILO
        ...
        INNER JOIN "VAR_DB_NAME".RESUMOPEDIDO TBR ON 
            TBR.IDRESUMOPEDIDO = TBDP.IDRESUMOPEDIDO AND TBR.STCANCELADO = 'False' AND TBDP.STCANCELADO = 'False'
        ...
        LEFT JOIN "VAR_DB_NAME".EMPRESA TBEMP ON
    ...
    if(idLista){
        query +=  ` AND TBHP.IDRESUMOLISTAPRECO IN (${idLista}) `;
    }
