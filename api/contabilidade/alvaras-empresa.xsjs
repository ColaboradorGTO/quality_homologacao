...
function getListaAlvarasEmpresa(idEmpresa) {
    ...
    let regs = api.sqlQuery(query, idEmpresa);
    
    const map = new Map();

    regs.forEach(registro => {
      const { IDALVARA, DESCRICAOALVARA, IDVINCULO } = registro;
      ...
      IDVINCULO && map.get(IDALVARA).ITEMS.push(registro);
    });
...
function montarResponse(response){
    ...
    for(let registro of data){
        ...
        let LISTA_GERENTES = getListaGerentesEmpresa(IDEMPRESA);
        let LISTA_SUPERVISORES = getSupervisorEmpresa(IDFUNCIONARIOSUPERVISOR)
        let objEmpresa = Object.assign({}, registro, { LISTA_GERENTES, LISTA_SUPERVISORES, LISTA_ALVARAS });
