...
function obterPCJ(idMovimento) {
	...
	var query = ' SELECT ' +
    	...
    	'   (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "VAR_DB_NAME".VENDAPAGAMENTO tbvp INNER JOIN "VAR_DB_NAME".VENDA tbv1 ON tbvp.IDVENDA=tbv1.IDVENDA WHERE tbv1.IDMOVIMENTOCAIXAWEB = \'' + idMovimento +'\' AND tbv1.STCANCELADO = \'False\' AND (tbvp.STCANCELADO = \'False\' OR tbvp.STCANCELADO IS NULL) AND (tbvp.NOAUTORIZADOR =\'Credsystem\' OR tbvp.NOAUTORIZADOR =\'CREDSYSTEM\' OR tbvp.NOAUTORIZADOR =\'PL\') AND (tbvp.DSTIPOPAGAMENTO !=\'GIRO PREMIADO\') AND (tbvp.NPARCELAS = 7 OR tbvp.NPARCELAS = 8)) AS TOTALPCJ78, '+
    	'   (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "VAR_DB_NAME".VENDAPAGAMENTO tbvp INNER JOIN "VAR_DB_NAME".VENDA tbv1 ON tbvp.IDVENDA=tbv1.IDVENDA WHERE tbv1.IDMOVIMENTOCAIXAWEB = \'' + idMovimento +'\' AND tbv1.STCANCELADO = \'False\' AND (tbvp.STCANCELADO = \'False\' OR tbvp.STCANCELADO IS NULL) AND (tbvp.NOAUTORIZADOR =\'Credsystem\' OR tbvp.NOAUTORIZADOR =\'CREDSYSTEM\' OR tbvp.NOAUTORIZADOR =\'PL\') AND (tbvp.DSTIPOPAGAMENTO !=\'GIRO PREMIADO\')) AS TOTALPCJ18 '+
        ' FROM ' +
...
function obterVenda(idMovimento) {
	...
	var query = ' SELECT ' +
    	...
    	'   SUM( tbv.VRRECCARTAO ) AS TOTALVENDIDOCARTAO, ' +
    	'   (SELECT IFNULL (SUM(tbvp.VALORRECEBIDO),0) FROM "VAR_DB_NAME".VENDAPAGAMENTO tbvp INNER JOIN "VAR_DB_NAME".VENDA tbv1 ON tbvp.IDVENDA=tbv1.IDVENDA WHERE tbv1.IDMOVIMENTOCAIXAWEB = \'' + idMovimento +'\' AND tbv1.STCANCELADO = \'False\' AND (tbvp.STCANCELADO = \'False\' OR tbvp.STCANCELADO IS NULL) AND tbvp.NOTEF = \'POS\' AND (tbvp.DSTIPOPAGAMENTO!=\'PIX\')) AS TOTALVENDIDOPOS, '+
...
function fnHandleGet(byId) {
    ...
    if(dataFechamento) {
        query = query + ' AND (tbmc.DTABERTURA  BETWEEN \'' + dataFechamento + ' 00:00:00\' AND \'' + dataFechamento + ' 23:59:59\') AND tbmc.ID > \'0\'';
    }
    
    query = query + 'GROUP BY tbmc.ID, tbc.IDCAIXAWEB, tbmc.IDOPERADOR,tbf.NUCPF,tbc.DSCAIXA, tbf.NOFUNCIONARIO, tbc.IDEMPRESA, tbmc.STFECHADO, tbmc.DTABERTURA, tbmc.VRRECDINHEIRO ';
