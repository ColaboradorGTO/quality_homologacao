...
function fnHandleGet(byId){
    ...
    var idNotaParaVinc = $.request.parameters.get("idNotaParaVinc");
    ...
    if (idNota) {
        query = query + ' And  tbvpn.IDRESUMOENTRADA = \'' + idNota + '\' ';
        query = query + ` And  tbvpn.STATIVO = 'True' `;
    }
    
    if (idNotaParaVinc) {
        query = query + ' And  tbvpn.IDRESUMOENTRADA != \'' + idNotaParaVinc + '\' ';
        query = query + ` And  tbvpn.STATIVO = 'False' `;
    }
