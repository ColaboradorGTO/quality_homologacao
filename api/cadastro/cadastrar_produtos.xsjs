...
function atualizaDetalheProdutoPedido(idDetProdPedido){
    ...
    var query = 'UPDATE "VAR_DB_NAME"."DETALHEPRODUTOPEDIDO" SET ' + 
        ...
        ' INNER JOIN "VAR_DB_NAME"."DETALHEPEDIDO" AS DetPed ON DETALHEPRODUTOPEDIDO.IDDETALHEPEDIDO = DetPed.IDDETALHEPEDIDO ' + 
        ' WHERE DetPed.IDRESUMOPEDIDO =  \'' + idDetProdPedido + '\' ';
    ...
    var query2 = 'UPDATE "VAR_DB_NAME"."RESUMOPEDIDO" SET ' + 
        ' RESUMOPEDIDO.IDANDAMENTO = 5 ' + 
        ' WHERE RESUMOPEDIDO.IDRESUMOPEDIDO =  \'' + idDetProdPedido + '\' ';
    
    var atualizadorDeAndamento = conn.prepareStatement(api.replaceDbName(query2));
    atualizadorDeAndamento.execute();
    
    atualizadorDeStatus.close();
    atualizadorDeAndamento.close();
	conn.commit();
} 
...
function execute () {
    ...
    var queryProdutos =  'SELECT \
             ...
             WHERE \
            	detprodped.STCANCELADO = \'False\' and (detprodped.STCADASTRADO = \'False\' or detprodped.STCADASTRADO IS NULL) and DETALHEPEDIDO.IDRESUMOPEDIDO = ? ';
        ...
        if(bodyJson.length > 0){
        	...
        	var lstDeProdutos = api.sqlQuery(queryProdutos,registro.IDRESUMOPEDIDO);
        	for (var i = 0; i < lstDeProdutos.length; i++) {
        	    var registroProd = lstDeProdutos[i];
        	    
        	    if(registroProd.STREPOSICAO !== 'True' ){
                    ...
                    var idProduto = api.executeScalar(queryIdProduto,1);
                        ...
                        pStmt.setString(33, registroProd.IDFORNECEDOR);
            ...
            atualizaDetalheProdutoPedido(registro.IDRESUMOPEDIDO);
        }
