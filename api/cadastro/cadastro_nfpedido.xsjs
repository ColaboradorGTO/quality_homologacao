...
function fnIncluirDetalheProdutosDanfePedido(idResumoEntrada, lstDet, lstDetEstoque, conn){
	...
	for (var i = 0; i < lstDet.length; i++) {
		...
		pStmt.setString(4, registro.CPROD);
...
function fnHandleGet(byId) {
    ...
    if ( byId ) {
        ...
        if(dataPesquisaInicio && dataPesquisaFim) {
            query += query + ` AND ((DEMI BETWEEN '${dataPesquisaInicio} 00:00:00' AND '$dataPesquisaFim} 23:59:59') OR (DTCADASTRO BETWEEN '${dataPesquisaInicio} 00:00:00' AND '${dataPesquisaFim} 23:59:59'))`;
        }
        
        if(numSerie && numNFE) {
            query += ` AND SERIE = ${numSerie} AND  NNF = ${numNFE}`;
        }
        
         if(chaveNFE) {
            query += ` AND CHNFE = '${chaveNFE}' `;
        }
	...
	for (let i = 0; i < bodyJson.length; i++) {
        ...
        if(CodNNF.length >0 ){
           ...
           if(registro.IDRESUMOPEDIDO){
            ...
            fnIncluirDetalheProdutosDanfePedido(idResumoEntrada, registro.detProdNFE, registro.detProdutoRecepcao, conn);
	}
