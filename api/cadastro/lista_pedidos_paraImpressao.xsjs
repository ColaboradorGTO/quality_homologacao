...
function fnHandleGet(byId) {
    ...
    var idOuDesc = $.request.parameters.get("idOuDesc");
    	
    var query =  ' SELECT' +
        '   tbrp.IDRESUMOPEDIDO AS IDPEDIDO, ' +
        '   tbrp.IDGRUPOEMPRESARIAL AS IDGRUPOPEDIDO, ' +
        '   tbrp.IDSUBGRUPOEMPRESARIAL AS IDSUBGRUPOPEDIDO, ' +
        '   IFNULL((SELECT DISTINCT MAX(DSFABRICANTE) FROM "VAR_DB_NAME".FABRICANTE LEFT JOIN "VAR_DB_NAME".DETALHEPEDIDO ON FABRICANTE.IDFABRICANTE=DETALHEPEDIDO.IDFABRICANTE WHERE DETALHEPEDIDO.IDRESUMOPEDIDO=tbrp.IDRESUMOPEDIDO AND DETALHEPEDIDO.STCANCELADO = \'False\'),\'\') AS FABRICANTE, ' +
    ...
    if(idOuDesc){
        query += `AND CONTAINS((tbrp.IDRESUMOPEDIDO, FN.NOFANTASIA, FN.NORAZAOSOCIAL, FN.NUCNPJ, FN.EEMAIL, FN.NUTELEFONE1, FN.ECIDADE, FC.IDFUNCIONARIO, FC.NOFUNCIONARIO ), '%${idOuDesc}%')`;
    }
