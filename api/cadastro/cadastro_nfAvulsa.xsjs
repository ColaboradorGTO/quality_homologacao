...
var slApi = $.import("quality.concentrador_homologacao.api.service-layer.notas-entrada.subir-nota-entrada", "slApi"); 
var msg;
var status= "success";
...
function fnVincularNotaAoPedido(idResumoPedido, idResumoEntrada, registro) {
    ...
    var idNotaFiscal = idResumoEntrada;
...
function fnIncluirDetalheProdutosDanfePedido(idResumoEntrada, lstDetProd, conn){
	...
	for (var i = 0; i < lstDetProd.length; i++) {
	    ...
	    lstDetProd[i].IDDETALHEENTRADA = queryId;
	    
		var registro = lstDetProd[i];
  ...
  // fnMovimentaInventario(lstDetProd, idResumoEntrada, conn)

}

function fnMovimentaInventario(lstDetProd, idResumoEntrada, conn) {
    ...
    for (let i = 0; i < lstDetProd.length; i++) {
        ...
        if(idMovExists.length === 0){
            ...
            qtdEntrada = parseInt(registro.QCOM) + parseInt(idMovExists[0].QTDENTRADA);
    ...
    var respSAP =  slApi.executeEntradaNfe(idResumoEntrada);
    
   if(respSAP == "True"){
        msg = "Nota Cadastrada e Migrada SAP com Sucesso!";
    } else {
        msg = respSAP.includes('Migração') ? respSAP : respSAP + " Corrija no SAP e tente subir a nota novamente!";
        status = "warning";
    }
}
...
function fnHandlePut() {
    ...
    for (var i = 0; i < bodyJson.length; i++) {
       ...
       // var respSAP = slApi.executeEntradaNfe(36);
        var respSAP = slApi.executeEntradaNfe(registro.IDRESUMOENTRADA);
       // return {respSAP}
        if(respSAP == "True"){
            msg = "Nota Cadastrada e Migrada para o SAP com Sucesso!";
            status = "success"; 
        } else {
            msg = respSAP//.includes('Migração') ? respSAP : respSAP + " Corrija no SAP e tente subir a nota novamente! " + registro.IDRESUMOENTRADA;
            status = "warning"; 
        }
    }
...
function fnHandlePost() {
    var conn = $.db.getConnection();
   /* var respSAP = slApi.executeEntradaNfe(20);
