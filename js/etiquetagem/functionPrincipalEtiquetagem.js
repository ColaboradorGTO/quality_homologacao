...
async function ListaProdutosEtiqueta() {
    try{
        ...
        retornoListadePreçosGrupos(respListas);
        
        let respEmpresas = await ajaxGetAllData(`api/empresa.xsjs?`, false).catch((error)=>{ throw error });
        retornoListadePreçosLojas(respEmpresas);
...
async function pesquisaProdutos() {
    ...
    let tipoLista = $('#idListaPreco').select2('data')[0]?.title;
    let idLista = tipoLista !== 'LOJA' ? $('#idListaPreco').val() : "";
    let idEmpresa = tipoLista == 'LOJA' ? $('#idListaPreco').val() : "";
    
    if ((idLista || idEmpresa) && (idProduto || descProd || codBarras)) {
        try {
            let urlApi = `api/produtos/lista-produtos-para-etiqueta.xsjs?idLista=${idLista}&idEmpresa=${idEmpresa}&id=${idProduto}&descProd=${descProd}&codeBars=${codBarras}`;

            let resp = await ajaxGetAllData(`${urlApi}`).catch(erro => erro);
