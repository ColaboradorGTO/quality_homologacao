...
// ? /////////////////////////////////////// INICIO ROTINA IMPRESSÃO ETIQUETAS DO FORNECEDOR CONSULTA NO SAP ////////////////////////////////////////////////////////////////{
/*
...
async function ListaProdutosEtiqueta() {
    try{
        ...
        await $.get("cadastro_action_ProdEtiqueta_fornecedor.html", function (resp) {
            ...
            $('#idPedido').on('keypress', (e) => { if (e.keyCode == 13) pesquisaProdutos() });

        }).catch((error) => { throw error });
        
        await montarSelectBuscaDinamicaFornecedores().catch((error) => { throw error });

        /*await ajaxGetAllData(`api/produtos/listas-de-precos-SAP.xsjs?`, false)
...
async function pesquisaProdutos() {
    try {
        ...
        let idFornecedor = $('#idforn').val();//4501//await getCurrentUser()?.user?.id || ''
        let idPedido = $('#idPedido').val() || "";

        if(!Number(idFornecedor)){
            return msgWarning('Usuario não encontrado, faça o logoff e tente novamente!');
        }

        if (true ){//idPedido) {
            await ajaxGetAllData(`api/produtos/lista-produtos-etiqueta-por-pedido-SAP.xsjs?idResumoPedido=${idPedido}&idFornecedor=${idFornecedor}`)
                    .then(retornoPesquisaProdutos)
                    .catch(error => { throw error; });
        } else{
            return msgWarning('Digite o número do pedido e tente novamente!');
        }
...
function listarProdsParaImprimir() {
    ...
    $('#modalGenerico').on('keydown', pulaPreviewEtiqueta);
}
...
async function modalImpEtiqueta(stShow = true) {
    $('#modalImpEtiquetaProd').on('keydown', pulaPreviewEtiqueta);
        ...
        let cardEtiqueta = `
            <div class="etiqueta-card" ${paddingFirstElement}>
                ...
                <div class="divTamanho" style="display: flex; justify-content: space-between;">
                    ...
                    <div class="preco">
                        <h2 style="margin: 1px !important">${maskValorEmBRL(precoVenda)}</h2>
                    </div>
                </div>
    ...
    if (!stShow) {
        impEtiquetaProdutos();
        document.removeEventListener('keydown', pulaPreviewEtiqueta);
    }
...
// ? /////////////////////////////////////// FIM ROTINA IMPRESSÃO ETIQUETAS DO FORNECEDOR CONSULTA NO SAP ////////////////////////////////////////////////////////////////